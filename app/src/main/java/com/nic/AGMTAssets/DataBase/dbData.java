...
public class dbData {
    ...
    public RoadListValue insertHabitation(RoadListValue kvvtSurvey) {

        ContentValues values = new ContentValues();
        values.put(AppConstant.DISTRICT_CODE, kvvtSurvey.getPmgsyDcode());
        values.put(AppConstant.BLOCK_CODE, kvvtSurvey.getPmgsyBcode());
        values.put(AppConstant.PV_CODE, kvvtSurvey.getPmgsyPvcode());
        values.put("habitation_code", kvvtSurvey.getPmgsyHabcode());
        values.put(AppConstant.HABITATION_NAME, kvvtSurvey.getPmgsyHabName());
        values.put(AppConstant.HABITATION_NAME_TA, kvvtSurvey.getPmgsyHabNameTa());

        long id = db.insert(DBHelper.HABITATION_TABLE,null,values);
        Log.d("Inserted_id_habitation", String.valueOf(id));

        return kvvtSurvey;
    }
    public RoadListValue insertAGMTForm(RoadListValue kvvtSurvey) {
    ...
    public ArrayList<RoadListValue> getAll_Habitation(String dcode, String bcode) {
        ...
        try {
            cursor = db.rawQuery("select * from "+DBHelper.HABITATION_TABLE+" where dcode = "+dcode+" and bcode = "+bcode+" order by habitation_name asc",null);
            // cursor = db.query(CardsDBHelper.TABLE_CARDS,
            //       COLUMNS, null, null, null, null, null);
            if (cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    RoadListValue card = new RoadListValue();
                    card.setPmgsyDcode(Integer.parseInt(cursor.getString(cursor
    ...
    public ArrayList<RoadListValue> getVillageAll_Habitation(String dcode, String bcode,String pvcode) {

        ArrayList<RoadListValue> cards = new ArrayList<>();
        Cursor cursor = null;

        try {
            cursor = db.rawQuery("select * from "+DBHelper.HABITATION_TABLE+" where dcode = "+dcode+" and bcode = "+bcode+" and pvcode = "+pvcode+" order by habitation_name asc",null);
            // cursor = db.query(CardsDBHelper.TABLE_CARDS,
            //       COLUMNS, null, null, null, null, null);
            if (cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
    ...
    public ArrayList<RoadListValue> getAgmtForm() {

        ArrayList<RoadListValue> cards = new ArrayList<>();
        Cursor cursor = null;

        try {
            cursor = db.rawQuery("select * from "+DBHelper.AGMT_FORM_TABLE,null);
            // cursor = db.query(CardsDBHelper.TABLE_CARDS,
            //       COLUMNS, null, null, null, null, null);
            if (cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    card.setForm_id(cursor.getString(cursor
                            .getColumnIndexOrThrow("form_id")));
                    card.setForm_number(cursor.getString(cursor
